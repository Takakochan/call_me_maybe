...
# CallMeMaybe 
### Constrained Decoding for LLM Function Calling
...
**No inference libraries.** The constrained-decoding logic is written explicitly against
raw logits rather than delegated to a library. This is a subject requirement, but it has a
technical consequence worth stating: masking operates on the full logit vector, so the
technique requires local weights. A hosted API only returns the token it already selected.
...
- **Tokenizer reimplementation and public implementation** byte-level BPE implemented from the vocabulary and merge
  files exposed by the SDK, removing the direct dependency on `encode` in the main code
  path. Verified against the SDK's own tokenizer (see `src/bpe_tokenizer.py`'s `__main__`
  block) and covered by unit tests in `test/test_bpe_tokenizer.py`.
- **Performance optimization (KV cache)** `Small_LLM_Model.get_logits_from_input_ids`
  transparently reuses the previous call's `past_key_values` whenever the new `input_ids`
  extend the previous ones, so only the newly appended tokens are run through the model
  instead of the full sequence every time. Falls back to a full forward pass when the input
  does not extend the cached prefix (a new prompt, or backtracking). Implemented inside
  `llm_sdk/llm_sdk/__init__.py`, since the public `get_logits_from_input_ids` signature is
  the only thing `src/` is allowed to depend on and the cache state is private to the SDK.
- **Comprehensive test suite** `test` folder contains test suite. 
...
- [**Transformer Architecture**](https://waylandz.com/llm-transformer-book-en/)
- [**The First 256 Characters Of UTF-8**](https://www.mrinitialman.com/HTMLTutorial/Chapters/Appendices/Appendices-Characters.html)
- [**GPT-2 Tokenizer (BPE: Byte-Pair Encoding)**](https://zenn.dev/7shi/books/20260319-gpt2/viewer/03_tokenizer)
- [**GPT2 OPENAI open source**](https://github.com/openai/gpt-2)
